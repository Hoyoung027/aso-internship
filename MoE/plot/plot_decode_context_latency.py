#!/usr/bin/env python3
"""Batch 1 Decode 구성을 입력 길이별 percentage와 millisecond로 그린다."""
...
DEFAULT_PERCENT_OUTPUT = PLOT_DIR / "figures" / "decode_b1_composition_percent.png"
DEFAULT_TIME_OUTPUT = PLOT_DIR / "figures" / "decode_b1_latency_ms.png"
BAR_WIDTH = 0.66
COLORS = {
    "e2e": "#cdb4db",
    "attention": "#3a86ff",
...
def load_rows(path: Path) -> list[dict[str, str]]:
    with path.open(newline="", encoding="utf-8") as file:
        rows = [
            ...
            if row["phase"] == "decode" and int(row["batch_size"]) == 1
        ]
    rows.sort(key=lambda row: int(row["input_tokens_per_sequence"]))
    if not rows:
        raise ValueError(f"No batch-1 Decode rows found in {path}")
    return rows
...
def main() -> int:
    ...
    draw_stacked(
        ...
        title="GPT-OSS 20B Decode GPU Time Composition (Batch Size = 1)",
        ylabel="Normalized time (% of Pure Decode GPU total)",
    ...
    draw_stacked(
        ...
        title="GPT-OSS 20B Decode Latency by Context Length (Batch Size = 1)",
        ylabel="Latency (ms)",
